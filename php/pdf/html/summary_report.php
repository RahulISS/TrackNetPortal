...
<?php foreach($this->fileData as $weekIndex => $fileData):?>
	...
	<table width="100%">
		...
		<tbody>
			<?php foreach(['AM','PM','Daily','Weekly'] as $classes):?>
				<?php if(isset($fileData[$classes]) && count($fileData[$classes])>0):?>
					<tr>
						<th align="left"><?= $classes;?></th>
						<?php echo str_repeat("<td class='empty'>&nbsp;</td>", $this->weekLimit);?>
					</tr>
					<tr>
						...
						<?php foreach ($dateList as $key => $date):?>
							<?php foreach ($this->request->options->sensorText as $sen => $lab):?>
								<?php if(isset($fileData[$classes][$date][$sen])):?>
									<td align="left"><?= round(array_sum($fileData[$classes][$date][$sen]),2);?></td>
								<?php endif;?>
							<?php endforeach;?>
						<?php endforeach;?>
					</tr>
					<tr>
						...
						<?php foreach ($dateList as $key => $date):?>
							<?php foreach ($this->request->options->sensorText as $sen => $lab):?>
								<?php if(isset($fileData[$classes][$date][$sen])):?>
									<td align="left"><?= round(max($fileData[$classes][$date][$sen]),2);?></td>
								<?php endif;?>
							<?php endforeach;?>
						<?php endforeach;?>
					</tr>
					<tr>
						...
						<?php foreach ($dateList as $key => $date):?>
							<?php foreach ($this->request->options->sensorText as $sen => $lab):?>
								<?php if(isset($fileData[$classes][$date][$sen])):?>
									<td align="left"><?= round(min($fileData[$classes][$date][$sen]),2);?></td>
								<?php endif;?>
							<?php endforeach;?>
						<?php endforeach;?>
					</tr>
					<tr class="borbot">
						...
						<?php foreach ($dateList as $key => $date):?>
							...
							<?php endforeach;?>
						<?php endforeach;?>
						<?php if($needToadd>0 || $classes=='Weekly') echo str_repeat("<td class='empty'>&nbsp;</td>", $classes=='Weekly'? 6 :$needToadd);?>
					</tr>
				<?php endif;?>
			<?php endforeach;?>
		</tbody>
