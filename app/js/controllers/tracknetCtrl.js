angular
  .module("tracknetCtrl", []).service('LocationDataService', function () {
    this.locationData = {};
  }).controller("tracknetController", function ($window, $scope, $rootScope, $http, Data, $q, $timeout, $interval, $filter, apiBaseUrl, portalId, LocationDataService) {
    ...
    $scope.clockTime = function () {
      $scope.time = moment().tz(localStorage.getItem("setTimeZone")).format("h:mm:ss a");
      $scope.date = moment().tz(localStorage.getItem("setTimeZone")).format("ddd, MMM Do YYYY");
    };
    ...
    $scope.loadData = function (initset, customParam = "") {
      ...
      if (customParam == '') {
        var apiUrl = $scope.isFirstLoad ? apiBaseUrl + "newtraknetApiList?portalId=" + portalId : apiBaseUrl + "newtraknetApiList/" + localStorage.getItem("singleDate") + "?portalId=" + portalId;
      } else {
        var apiUrl = customParam ? apiBaseUrl + "newtraknetApiList?portalId=" + portalId : apiBaseUrl + "newtraknetApiList/" + localStorage.getItem("singleDate") + "?portalId=" + portalId;
      }


      $http.get(apiUrl, { headers: customeHeader }).then(function (res) {
        ...
        for (var i = response.length - 1; i >= 0; i--) {
          ...
          if (!existingObject) {
            ...
            var closestValue = closest({ 'al1': parseInt(data.distanceAlert.alert1), 'al2': parseInt(data.distanceAlert.alert2), 'al3': parseInt(data.distanceAlert.alert3) }, data.point.height);
            ...
            if (parseInt(data.point.height) <= 400) {
              ...
              var dis_color = "Red";
            } else if (parseInt(data.point.height) > 400 && closestValue) {
            ...
            if (data.point.angle >= 5) {
              ...
              if (data.distanceAlert.alert1 != null && distanceValue < data.distanceAlert.alert1 && data.distanceAlert.alarmFirstCheck != null && data.distanceAlert.alarmFirstCheck == 1 && parseInt(data.distanceAlert.alert1) == closestValue) {
                var markerShape = "circle";
              }
              if (data.distanceAlert.alert2 != null && distanceValue < data.distanceAlert.alert2 && data.distanceAlert.alarmSecondCheck != null && data.distanceAlert.alarmSecondCheck == 1 && parseInt(data.distanceAlert.alert2) == closestValue) {
                var markerShape = "square";
              }

              if (data.distanceAlert.alert3 != null && distanceValue < data.distanceAlert.alert3 && data.distanceAlert.alarmThirdCheck != null && data.distanceAlert.alarmThirdCheck == 1 && parseInt(data.distanceAlert.alert3) == closestValue) {
            ...
            var convertedPoint = {
              ...
              product_serialNumber: data.product.id_serial,
              ...
              alarmFirstCheck: data.distanceAlert.alarmFirstCheck,
              alarmSecondCheck: data.distanceAlert.alarmSecondCheck,
              alarmThirdCheck: data.distanceAlert.alarmThirdCheck,
              totalAlerts: {
                'al1': data.distanceAlert.alert1,
                'al2': data.distanceAlert.alert2,
                'al3': data.distanceAlert.alert3
              },
    ...
    function getMarkerColor(info, customParam = "") {
      let alertArr = [];
      if (info.alarmFirstCheck == 1) {
        alertArr.push(info.totalAlerts.al1);
        var dict = {
          'al1': info.totalAlerts.al1,
          'al2': info.totalAlerts.al2,
          'al3': info.totalAlerts.al3
        }
      }
      if (info.alarmSecondCheck == 1) {
        alertArr.push(info.totalAlerts.al2);
        var dict = {
          'al1': info.totalAlerts.al1,
          'al2': info.totalAlerts.al2,
          'al3': info.totalAlerts.al3
      ...
      if (info.alarmThirdCheck == 1) {
        alertArr.push(info.totalAlerts.al3);
        var dict = {
          'al1': info.totalAlerts.al1,
          'al2': info.totalAlerts.al2,
          'al3': info.totalAlerts.al3
    ...
    function createMarker(info) {
      ...
      const singaporeTime = moment(ttemp).tz(localStorage.getItem("setTimeZone"));
      ...
      marker.content =
        ...
        " mm<br>" +
        "<b>Relative Distance: </b>" + info.relative_distance + ' %<br>' +
        "<b>Angle: </b>" +
        info.angle +
        " deg<br>" +
        "<b>Temperature: </b>" +
        ...
        " dBm<br>" +
        "<b>Battery Status: </b>" +
        parseFloat(info.batteryVolt).toFixed(2) +
        " V<br>" +
        "</div>";
      ...
      marker.sensorContent =
        ...
        '<li> <span class="data_name"><i class="fa fa-gg-circle" aria-hidden="true"></i> <span>Relative Distance: </span>' + info.relative_distance + ' % <span class="data-date"></span> </li>' +
    ...
    $scope.alertClick = function (alert) {
      ...
      if (document.getElementById("myDiv"))
        document.getElementById("myDiv").innerHTML = $scope.locationData[alert.product_serialNumber].marker.sensorContent;
      $scope.updateChartConfig();
      let initset = true;
      tracknetMap.infoWindow.close();
    ...
    function alarmApi() {
      ...
      $http.get(apiBaseUrl + "newtraknetApiList?portalId=" + portalId, { headers: customeHeader }).then(function (res) {
        ...
        for (var i = responses.length - 1; i >= 0; i--) {
          ...
          var existingObject = alarmArrayData.find(
            (obj) => obj.serialNumber === objectId
          );
          ...
          if (!existingObject) {
            ...
            if (data.point.angle > 5) {
              ...
              if (data.distanceAlert.alert1 != null && data.point.height < data.distanceAlert.alert1 && data.distanceAlert.alarmFirstCheck != null && data.distanceAlert.alarmFirstCheck == 1 && parseInt(data.distanceAlert.alert1) == closestValue) {
                var markerShape = "circle";
              }
              if (data.distanceAlert.alert2 != null && data.point.height < data.distanceAlert.alert2 && data.distanceAlert.alarmSecondCheck != null && data.distanceAlert.alarmSecondCheck == 1 && parseInt(data.distanceAlert.alert2) == closestValue) {
                var markerShape = "square";
              }

              if (data.distanceAlert.alert3 != null && data.point.height < data.distanceAlert.alert3 && data.distanceAlert.alarmThirdCheck != null && data.distanceAlert.alarmThirdCheck == 1 && parseInt(data.distanceAlert.alert3) == closestValue) {
                var markerShape = "triangle";
              }
            }
            /** ends */
            ...
            const singaporeCurrentTime = moment(currentDate).tz(localStorage.getItem("setTimeZone"));
            ...
            var convertedPoint = {
              ...
              message: msg,
              aTreeNode_textLabel: data.point.textLabel,
              height: parseFloat(data.point.height),
            ...
            alarmArrayData.push(convertedPoint);
          }
        }
        ...
        for (var i = 0; i < alarmArrayData.length; i++) {
          var data = alarmArrayData[i];
        ...
        for (var i = 0; i < uniqueDataCount.length; i++) {
          ...
          //orange color count
          if (uniqueDataCount[i].angle < 5 && ((distanceDataCValue < parseInt(uniqueDataCount[i].totalAlerts.al1) && uniqueDataCount[i].alarmFirstCheck == 1) || (distanceDataCValue < parseInt(uniqueDataCount[i].totalAlerts.al2) && uniqueDataCount[i].alarmSecondCheck == 1) || (distanceDataCValue < parseInt(uniqueDataCount[i].totalAlerts.al3) && uniqueDataCount[i].alarmThirdCheck == 1))) {
            if (distanceDataCValue > 400) {
              $scope.realtimesummery.series[0].data[1].y++;
            }
          }
          //green color count
          else if (uniqueDataCount[i].angle < 5 && (uniqueDataCount[i].distance > 400 || (uniqueDataCount[i].alarmFirstCheck == 1 || uniqueDataCount[i].alarmSecondCheck == 1 || uniqueDataCount[i].alarmThirdCheck == 1 && distanceDataCValue > parseInt(uniqueDataCount[i].totalAlerts.al3)) && (distanceDataCValue > parseInt(uniqueDataCount[i].totalAlerts.al1) || distanceDataCValue > parseInt(uniqueDataCount[i].totalAlerts.al2)))) {
    ...
    $scope.updateChartConfig = function () {
      ...
      if ($scope.device[$scope.serialNo] != undefined) {
        for (let j = 0; j < $scope.device[$scope.serialNo].length; j++) {
          ...
          var relativeDistance = eachEntry.relative_distance
      ...
      $scope.meterChartConfig = {
        options: {
          ...
          tooltip: {
            shared: true,
            formatter: function () {
              return toolDateFormaterFunction(this, "number", null);
            },
            crosshairs: {
              color: "black",
              dashStyle: "solid",
    ...
    function formatRDToolTip(pointClicked) {
      ...
      $val_full = Math.round((((data.empty - data.full) - (data.height - data.full)) / (data.empty - data.full)) * 100);
      if ($val_full < 0) {
        $val = 0;
      } else if ($val_full > 100) {
        $val = 100;
