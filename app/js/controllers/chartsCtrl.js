angular
  ...
  .controller("chartsController", function ($scope, $rootScope, Data, $q, $http, $timeout, $interval, $window, apiBaseUrl) {
    ...
    $scope.myCustomDownload = function () {
      ...
      let downloadUrl = apiBaseUrl + "download_chart?aTreeNodeId=" + nodeId + "&sensorId=" + sensorId + "&startDate=" + sDate + "&endDate=" + eDate + "&fold=actual";
    ...
    $scope.$watch("selected", function (node) {
      ...
      if (!$scope.showScatterPlot) {
        ...
        $scope.tableStats[currentLegend].currentMeasurement = $scope.tableStats[currentLegend].measurements.find(sensor => sensor.id_name === $scope.change);
