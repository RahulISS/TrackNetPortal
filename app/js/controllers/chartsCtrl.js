angular.module('chartsCtrl', [])
    ...
    .controller('chartsController', function ($scope, $rootScope,Data,$q,$http,$timeout,$interval,$window) {
        ...
        function isShowFlowReportButton() { 
            ...
            $scope.flowReportRangeStartDate = $rootScope.storage.chartsRangeStartDate;
            $scope.flowReportRangeEndDate = $rootScope.storage.chartsRangeEndDate;
            let frmDate = moment($scope.flowReportRangeStartDate).format(skySparkFormat);
            let toDate = moment($scope.flowReportRangeEndDate).format(skySparkFormat);
            for(var i=0;i<$scope.tableStats.length;i++) {
                if ($scope.tableStats[i].pointId == null || $scope.tableStats[i].pointId == "null") continue;
                ...
                $q.all(promises_data).then(function (responses) {
                    ...
                    for (let j = 0; j < responses.length; j++) {
                        for(let k = 0; k < responses[j].data.rows.length; k++) {
                            if(responses[j].data.rows[k].productModel == 'TWP-FSPRO-2AV-RAD-8C') {
                                $scope.flowReportBtn = true;
                                $scope.flowReportNode = $scope.tableStats[responses[j].idx].title;
                                j=responses.length;
                                break;
                            }
                        }
        ...
        $scope.selectFlowRepSensor = function(idx) {
            for(let i=0;i<$scope.flowSensorData.length;i++) $scope.flowSensorData[i].checked = (i == idx);
        ...
        $scope.toggleFlowRepModel = function(reset) {
            ...
            if($scope.flowReportEnable) {
                $scope.flowReportRangeStartDate = $rootScope.storage.chartsRangeStartDate;
                $scope.flowReportRangeEndDate = $rootScope.storage.chartsRangeEndDate;
                changeDatePickerShow("#flowReportRange", $scope.flowReportRangeStartDate, $scope.flowReportRangeEndDate, dataPickerFormat);
        ...
        $scope.exportFlowReport = function(caseType)
        {
            ...
            let dateList = [];
            dateList.push(moment($scope.flowReportRangeStartDate).format('YYYY-MM-DD'));
            dateList.push(moment($scope.flowReportRangeEndDate).format('YYYY-MM-DD'));
            ...
            Data.generatePdf(
                ...
                if(caseType == 1) {
                    ...
                    link.setAttribute("download", $scope.flowReportTypeSel.label+'.pdf');
        ...
        function (start, end) {
            if (start.isSame(end, "day")) $('#flowReportRange span').html(start.format(dataPickerFormat));
            else $('#flowReportRange span').html(start.format(dataPickerFormat) + ' - ' + end.format(dataPickerFormat));
        ...
        $scope.changeFlowDateRange = function (direction) {
            var tempA = changeDateRange("#flowReportRange", direction, $scope.flowReportRangeStartDate, $scope.flowReportRangeEndDate, dataPickerFormat);
        ...
        $scope.switchMarkers = function (page) {
            // console.log($scope.chartStatusSet[page].markers, "marker");
        ...
        $scope.myCustomDownload = function () {
            ...
            let downloadUrl ='https://dev-api-sg.tracwater.asia/api/v1/download_chart?aTreeNodeId='+nodeId+'&sensorId='+sensorId+'&startDate='+sDate+'&endDate='+eDate+'&fold=actual';
        ...
        $scope.checkstorage = function () {
            var isChecked = localStorage.getItem("isChecked") ? localStorage.getItem("isChecked") : "full";
          ...
          function loadData() {
            ...
            const promises_data = $scope.queriesArray.map(function (item) {
				...
				.then(function (reqResult) {
                    console.log(reqResult,'eqResult')
                    return {
                        'index': item.index,
                        'data': reqResult
                    };
            ...
            //console.log(data_completion,'promises_data sorb saxena')
            data_completion.then(function (responses) {
                if(responses.length === 0) return;
                $scope.tableViewData = [];
                //console.log(responses[0].data.data.data,'responses[0].data.data.data')
                ...
                for(let i = 0; i < responses.length; i++ ){
                    ...
                    console.log(values,'values saxena')
        ...
        function deselectAllTreeNodep(treeid) {
            $scope.selected = null;
            $(treeid).find('.jstree-disabled').each(function () {
...
function changeDatePickerShow(datapickerId, start, end, format) {
    ...
    if (moment(start).isSame(moment(end), "day"))
        $(datapickerId + ' span').html(moment(start).format(format));
    else $(datapickerId + ' span').html(moment(start).format(format) + ' - ' + moment(end).format(format));
    if(start!=null) $(datapickerId).data('daterangepicker').setStartDate(moment(start));
    if(end!=null) $(datapickerId).data('daterangepicker').setEndDate(moment(end));
}
