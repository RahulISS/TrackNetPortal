angular.module('mainCtrl', [])
    ...
    .controller('mainController', function ($scope,$rootScope, $http, $state,$q,Data,$location) {
        ...
        $rootScope.loadTree = function() {
            const query = $http.get('http://54.254.34.0/api/v1/chart')
                    ...
                    tree.forEach(sub => {
                        sub.children.forEach(subSub => {
                            subSub.children.forEach(el => {
                                queriesArray.push({
                                    'index': el._id,
                                    'query': `http://54.254.34.0/api/v1/atreenode-ref?productRef=${el._id}`
                                });
                            });
                        });
                    }); 
        ...
        function checkedRefData(e, edata) {
            let sensors = [];
              const query2 = $http.get('http://54.254.34.0/api/v1/sensortList')
              .then(function (response){
                    const data = response.data.data
                    for( let i = 0; i < data.length; i++){
                        if(data[i].unit != "1"){
